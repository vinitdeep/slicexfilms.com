...
import { FILMS } from '../app/(site)/films/films-data';
import { HOME } from '../app/(home)/home-data';
...
const merge = (key, data) => {
  ...
  const out = { ...base, ...data };
  // Services saved before the Services-page fields existed only carry the short
  // ones. Backfill per item by id so those saves keep the longer page copy;
  // a field the admin actually set (even to "") still wins.
  if (key === 'services' && Array.isArray(out.items)) {
    const byId = new Map((base.items || []).map((s) => [s.id, s]));
    out.items = out.items.map((s) => (byId.has(s.id) ? { ...byId.get(s.id), ...s } : s));
  }
  return out;
};
